...
code:`class Solution {
    ...
    public String addHex(String a, String b) {
        ...
        // Максимальная длина результата: maxLen + 1 (из-за возможного переноса)
        char[] result = new char[maxLen + 1];
        
        int p1 = a.length() - 1;
        int p2 = b.length() - 1;
        int carry = 0;
        int pos = maxLen; // начинаем заполнять с последнего индекса массива
        ...
        while (p1 >= 0 || p2 >= 0 || carry > 0) {
            ...
            result[pos--] = intToHex(sum % 16);
            carry = sum / 16;
        }
        
        // Если первый символ массива не заполнен (не было переноса в старший разряд)
        if (pos == maxLen) {
            // Переноса не было — берём подстроку с индекса 1
            return new String(result, 1, maxLen);
        } else {
            // Был перенос — возвращаем весь массив
