...
{id:"hf14",t:"Билеты по маршруту (те же объекты, порядок следования)",p:"HashMap",d:"средне",
desc:`У нас есть набор билетов — каждый билет это объект с полями ==from== (откуда) и ==to== (куда). Из всех билетов можно собрать ==единственный непрерывный маршрут==: петель и повторов городов нет, из каждого города не больше одного исходящего билета.

==Начало маршрута известно== — дан город ==start==, с которого турист выезжает первым.

Нужно вернуть ==те же объекты билетов== в порядке следования по маршруту (первая пара — билет из start, далее по цепочке).

Пример (идея как в JS):
\`\`\`
[
...
start = "NY"
\`\`\`
Порядок билетов: NY→London, London→Moscow, Moscow→SPb.`,
hint:`Построй HashMap: город from → сам объект билета. Тогда от start идёшь по цепочке map.get(current.to)… или проще: from → билет, и каждый шаг current = ticket.to.`,
code:`class Solution {
    ...
    /**
     * @param tickets непустой набор, образующий одну цепочку без циклов
     * @param start город начала маршрута (есть ровно один билет с ticket.from.equals(start))
     */
    public List<Ticket> orderTickets(List<Ticket> tickets, String start) {
        Map<String, Ticket> byFrom = new HashMap<>();
...
complexity:`Время: O(n) (n — число билетов; один проход для карты from→билет + один проход по цепочке ≤ n шагов), Память: O(n) (HashMap — до n записей «город from → ссылка на билет»; список ordered — n ссылок на те же объекты, новые билеты не создаём)`,
complexityExpl:`Каждый билет кладём в map один раз. Цикл идёт по уникальной цепочке — не более n итераций. Доп. память: карта и список указателей на исходные Ticket.`,
expl:`Инвариант задачи: граф «город → следующий город» — путь без ветвлений. Значит, для каждого from существует не больше одного билета. Карта byFrom хранит следующий шаг. Старт задан — идём: взяли билет из start, перешли в to, снова ищем билет, у которого from равен этому to, пока цепочка не кончится. Порядок в ordered совпадает с порядком перелётов.`,
ya:true,
repoSimilar:["hf6"]},
