...
{id:"hf14",t:"Билеты по маршруту (те же списки, порядок следования)",p:"HashMap",d:"средне",
desc:`Дан список билетов ==List<List<String>> tickets==: каждый билет — внутренний список из двух строк, ==get(0)== — город отправления, ==get(1)== — город прибытия.

Из всех билетов можно собрать ==единственный непрерывный маршрут==: петель и повторов городов нет, из каждого города не больше одного исходящего билета.

==Начало маршрута известно== — задана строка ==start== (город первого вылета).

Нужно вернуть ==те же объекты-списки билетов== (те же ссылки на внутренние списки) в порядке следования по маршруту.
