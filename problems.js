...
desc:`==Яндекс.== Даны два массива nums1 и nums2, каждый — список пар [время, значение]. Нужно ==сложить два графика== в один и вернуть итоговый график в виде списка пар [время, сумма значений], ==отсортированный по времени==.
...
steps:`1) sort(nums1), sort(nums2) по первой координате.
2) Два указателя и два «текущих» значения (изначально 0).
3) Пока не обработаны оба списка: time = min из голов; при совпадении времени с точкой — обновить значение и сдвинуть указатель.
4) В результат добавить [time, value1 + value2].`,
code:`import java.util.*;
