...
complexity:`Время: O(n), Память: O(1)`,
complexityExpl:`Один линейный проход по строке. Используются только несколько переменных (num, operator, last, sum), поэтому дополнительная память константная.`,
...
complexity:`Время: O(T log K), Память: O(T)`,
complexityExpl:`T — суммарная длина всех строк в кодпоинтах, K — число уникальных символов в строке (из-за TreeMap). Память уходит на хранилище групп и ключей.`,
...
complexity:`Время: O(n), Память: O(k)`,
complexityExpl:`Два линейных прохода по строке. k — число различных символов, которые хранятся в HashMap.`,
...
complexity:`Время: O(n), Память: O(w)`,
