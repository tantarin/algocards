...
// ===== HASHMAP =====
{id:"gh2",t:"Строгая симметрия по оси Y",p:"HashMap",d:"средне",
...
{id:"gh3",t:"LeetCode 356 — Line Reflection",p:"HashMap",d:"средне",
...
{id:"hf15",t:"LeetCode 2006. Count Number of Pairs With Absolute Difference K",p:"HashMap",d:"легко",
desc:`Дан массив целых чисел nums и число k. Нужно посчитать ==количество пар индексов== (i, j), где i < j и |nums[i] - nums[j]| == k.

Пример 1:
Ввод: nums = [1,2,2,1], k = 1
Вывод: 4
Пояснение: подходят пары (0,1), (0,2), (1,3), (2,3)
...
- 1 <= nums[i] <= 100
- 1 <= k <= 99`,
hint:`Идём слева направо и храним, сколько раз уже встречалось каждое число. Для текущего x новые пары дают только значения x - k и x + k, которые уже были раньше.`,
code:`class Solution {
    public int countKDifference(int[] nums, int k) {
        ...
        for (int num : nums) {
            pairs += freq.getOrDefault(num - k, 0);
            pairs += freq.getOrDefault(num + k, 0);
...
complexity:`Время: O(n), Память: O(u), где u — число различных значений`,
complexityExpl:`Для каждого элемента делаем O(1) обращений к HashMap: смотрим частоты num - k и num + k, затем обновляем частоту num. Итого один линейный проход O(n). Карта хранит до u разных чисел.`,
expl:`Ключевая идея: считать пары в момент, когда видим правый конец пары. Пусть текущий элемент равен num. Тогда все новые подходящие пары должны состоять из уже встреченного числа num - k или num + k, потому что только с ними модуль разности равен k.

Поэтому перед добавлением num в карту:
- прибавляем количество уже встреченных num - k
- прибавляем количество уже встреченных num + k

После этого увеличиваем freq[num]. Так каждая пара считается ровно один раз, потому что мы учитываем только элементы слева от текущего индекса.`,
lcSimilar:[{"n":2006,"t":"Count Number of Pairs With Absolute Difference K","h":"count-number-of-pairs-with-absolute-difference-k"}]},
...
desc:`Дан массив целых чисел nums. Нужно найти индекс элемента, такой что суммы всех элементов справа и слева от этого элемента равны.
...
desc:`Даны строки s и t. Нужно найти все индексы в строке s, с которых начинается подстрока — анаграмма строки t, и вернуть их в порядке возрастания. Считается, что символы могут быть любыми и заранее не известны (алфавит не ограничен), но на практике строки будут состоять только из символов ascii.
...
desc:`Дана строка s. Нужно посчитать количество подстрок, содержащих все уникальные символы исходной строки хотя бы по одному разу.
Решение должно потреблять не более чем O(k) дополнительной памяти, где k — число уникальных символов в s.
...
code:`class Solution {
    public int countComplete(String s) {
        long target = s.chars().distinct().count();

        int[] freq = new int[128];

        long haveCount = 0;  // сколько уникальных уже в окне
        int left = 0;
        int count = 0;
        for (int right = 0; right < s.length(); right++) {
            char rightChar = s.charAt(right);
...
code:`class Solution {
    public long[] slidingProduct(int[] arr, int k) {
        int n = arr.length;
...
{id:"st2",t:"LeetCode 739 Daily Temperatures (Прогноз потеплений)",p:"Stack",d:"средне",
...
// ===== HEAP / PQ =====
{id:"uf1",t:"Объединение графиков",p:"Heap / PQ",d:"средне",
...
// ===== SLIDING WINDOW =====
{id:"swd1",t:"LC 1438 Longest Continuous Subarray With Absolute Diff Less Than or Equal to Limit.",p:"Sliding Window",d:"сложно",
...
{id:"pse5",t:"Prefix Common Array (Общий префикс двух массивов)",p:"Prefix Sum Ext.",d:"средне",
...
{id:"tr6",t:"LC 103 · Binary Tree Zigzag Level Order Traversal",p:"Trees / DFS",d:"легко",
...
{id:"swd2",t:"Sliding Window Maximum",p:"Sliding Window",d:"сложно",
...
class Solution {
    public int[][] intersectSegments(int[][] segments1, int[][] segments2) {
        ...
        while (i < segments1.length && j < segments2.length) {
            ...
            //Интервал с меньшим концом полностью исчерпал все возможности пересечений. Его можно безопасно "выбросить" и двигать указатель.
...
desc:`Даны два массива ==a== и ==b==. Нужно найти минимальное значение ==abs(a[i] - b[j])== по всем парам индексов i, j.
...
// ===== SLIDING WINDOW =====
{id:"lc1446",t:"LC 1446 · Consecutive Characters",p:"Sliding Window",d:"легко",
...
// ===== HASHMAP =====
{id:"lc771",t:"LC 771 · Jewels and Stones",p:"HashMap",d:"легко",
