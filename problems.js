...
expl:`O(n·L) время и память, где L — максимальная длина строки. Все анаграммы дают одинаковый частотный ключ.`,
ya:true,
...
expl:`Граф исходящих рёбер — простой путь: у каждого from ровно один билет. Карта byFrom даёт следующий шаг. Старт задан: кладём билет с from == start, переходим в to, повторяем, пока есть исходящий билет. В ответе — те же list-объекты, что и во входе, только в порядке перелётов.`,
ya:true,
repoSimilar:["hf6"]},
...
Шаг 4: сжимаем left: freq[leftChar]++; если freq[leftChar] > 0, увеличиваем needCount; left++.
Повторяем, пока right не дойдёт до конца s.`,
ya:true,
...
expl:`Используем идею «отложенного сложения». Переменная last хранит последний терм с учётом * и /. Когда встречаем + или -, переносим last в sum и начинаем новый терм. Так соблюдаем приоритет операторов без стека.`,
ya:true},
...
expl:`DP строится снизу вверх: dp[i] = min(dp[i - x^2] + 1). Помимо значения минимума сохраняем, какое x взяли последним (chosen[i]). Потом идём от N назад по chosen и восстанавливаем сам список чисел.`,
ya:true},
...
expl:`Один обход в постпорядке: сначала получаем id левого и правого поддеревьев, затем ключ node.val + id слева + id справа. Одинаковые поддеревья дают один и тот же ключ и один id. Когда счётчик для id становится 2, добавляем узел в ответ (один представитель группы дубликатов).`,
ya:true},
...
expl:`Для каждого слова строим канонический ключ по частотам символов. В отличие от решения на 26 латинских букв, здесь ключ универсальный и корректен для mixed-алфавита и спецсимволов.`,
ya:true},
...
Если такого символа нет — вернуть специальное значение (например, '�' или пустую строку, в зависимости от сигнатуры).
...
expl:`Первый проход даёт частоты, второй — сохраняет исходный порядок и находит первый символ с частотой 1.`,
ya:true},
...
expl:`Из-за незначимости ведущих нулей цифра 0 не влияет на принадлежность к группе: 111 и 101010 оказываются эквивалентны (нули можно переставить в начало). Поэтому ключ строим только по частотам цифр 1..9.`,
ya:true},
...
expl:`Это классический sliding window с инвариантом: в окне не более k различных символов. Двигаем right, увеличиваем частоту. Если уникальных стало > k, двигаем left и уменьшаем частоты, удаляя символ из map при нуле. После восстановления инварианта обновляем максимум длины.`,
ya:true},
...
expl:`Ключевая идея: после сортировки нет смысла сравнивать каждый элемент со всеми — достаточно двигать указатель у меньшего числа, чтобы попытаться сократить разность.`,
ya:true},
...
expl:`Храним метки времени запросов в очереди. На getHits вычищаем из головы всё, что старше окна 300 секунд, после чего размер очереди и есть ответ.
Follow-up при очень большом количестве запросов в одну секунду: использовать 300 бакетов (times[300], hits[300]) по индексу timestamp % 300. При коллизии секунды перезаписываем бакет. Тогда hit — O(1), getHits — O(300)=O(1), память O(1).`,
ya:true,
p2:"Locked / Premium",
lcSimilar:[{"n":362,"t":"Design Hit Counter","h":"design-hit-counter"}],
repoSimilar:["ya4"]},
...
expl:`Идём по уровням слева направо, и в каждом уровне берём последний извлечённый узел — именно он виден справа.`,
ya:true},
...
expl:`Слияние «с хвоста» позволяет не затирать ещё не обработанные значения в nums1.`,
ya:true},
...
expl:`Перестановка имеет те же частоты символов. Значит, достаточно проверить все окна фиксированной длины на совпадение частот.`,
ya:true},
...
expl:`mins дублирует только значения, которые были минимумом на момент вставки, поэтому текущий минимум всегда на вершине mins.`,
ya:true},
...
expl:`Минимум находится в неотсортированной части. Сравнение с правой границей позволяет понять, где находится перелом.`,
ya:true},
...
expl:`На каждом шаге выбираем меньший текущий узел и двигаем соответствующий список вперёд.`,
ya:true},
