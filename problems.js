...
expl:`При добавлении A[i]: если countA[v] <= countB[v], то min(countA,countB) для этого v вырос на 1 — common++. Аналогично для B[i]. Время O(n) и память O(n) — асимптотически оптимально (нужно учесть каждый элемент). Если значения только из 1..n, HashMap можно заменить на int[n+1] — тот же порядок, меньше константа.`},
