...
steps:`Алгоритм Кана (обратный граф):
1. Строим reverse[v] — родители v в исходном графе. Считаем outdegree[u].
2. В очередь — вершины с outdegree == 0 (терминальные).
3. BFS: извлекаем node → safe[node]=true. Для каждого parent в reverse[node]: outdegree[parent]--, если 0 → в очередь.
4. Собираем все i где safe[i]==true.`,
code:`class Solution {
...
// ===== ONE PASS WITH STATE =====
{id:"sw6",t:"LC 1493. Longest Subarray of 1's After Deleting One Element",p:"One Pass with State",d:"средне",
...
==Что такое in-place==
Работать нужно ==в том же массиве arr==, без выделения второго массива под ответ (и без StringBuilder как основного хранилища результата). Символы, которые остаются после удаления смайликов, сдвигаются в начало массива: валидная часть — это ==arr[0], arr[1], …, arr[w−1]==.

==Что вернуть==
Метод возвращает целое число ==w== — ==новую логическую длину== массива (сколько символов осталось). Если w = 0, подходящих символов нет. Ячейки arr[w], arr[w+1], … с точки зрения задачи можно не смотреть (там может остаться «мусор» от старых данных).

Пример:
Ввод: ['a',':','-',')','b']
Вывод по смыслу: ['a','b'] (в начале arr, return w = 2)
...
steps:`1. In-place: read сканирует массив, write — куда пишем валидные символы.
2. Паттерн ':-)' / ':-(' — read += 3 и пропуск повторов той же скобки.
3. Иначе arr[write++]=arr[read++].
4. Вернуть write — новая логическая длина.`,
complexity:`Время: O(n), Память: O(1)`,
...
// ===== TWO POINTERS =====
{id:"tp31",t:"Valid Palindrome",p:"Two Pointers",d:"легко",
...
steps:`1. left=0, right=n-1.
2. Пропускать не-алфавитно-цифровые символы с обеих сторон.
3. Сравнить toLowerCase(left) и toLowerCase(right).
4. Если не совпадают — false. Иначе left++, right--.`,
complexity:`Время: O(n), Память: O(1)`,
...
3. sum == target → return. sum < target → left++. sum > target → right--.`,
complexity:`Время: O(n), Память: O(1)`,
...
{id:"tp33",t:"Reverse String",p:"Two Pointers",d:"легко",
...
steps:`1. slow=0, fast=1.
2. Если nums[fast] != nums[slow]: slow++, nums[slow] = nums[fast].
3. fast++ всегда.
4. Вернуть slow+1.`,
complexity:`Время: O(n), Память: O(1)`,
...
steps:`1. slow=0, fast=0.
2. Если nums[fast] != val: nums[slow] = nums[fast], slow++.
3. fast++ всегда.
4. Вернуть slow.`,
complexity:`Время: O(n), Память: O(1)`,
complexityExpl:`Один проход — O(n). In-place — O(1).`,
...
steps:`1. slow=0, fast=0.
2. Если nums[fast] != 0: nums[slow] = nums[fast], slow++.
3. После цикла — заполнить nums[slow..n-1] нулями.`,
complexity:`Время: O(n), Память: O(1)`,
...
steps:`1. i=0 (на s), j=0 (на t).
2. Если s[i] == t[j]: i++.
3. j++ всегда.
4. Вернуть i == s.length().`,
complexity:`Время: O(n), Память: O(1)`,
...
{id:"tp38",t:"Valid Palindrome II",p:"Two Pointers",d:"легко",
...
steps:`1. left + right с двух концов.
2. Если s[left] != s[right]: проверить isPalin(left+1, right) || isPalin(left, right-1).
3. Иначе left++, right--.`,
complexity:`Время: O(n), Память: O(1)`,
...
3. Двигать меньший: если height[l] < height[r] → left++, иначе → right--.`,
complexity:`Время: O(n), Память: O(1)`,
...
steps:`1. Сортировать nums.
2. Фиксировать nums[i] (пропускать дубликаты).
3. left=i+1, right=n-1: искать пару с суммой -nums[i].
4. При нахождении — пропускать дубликаты left и right.`,
complexity:`Время: O(n²), Память: O(1)`,
...
steps:`1. Сортировать, инициализировать closest первой тройкой.
2. Фиксировать i, left=i+1, right=n-1.
3. sum == target → return. Обновлять closest по минимальному |diff|.
4. sum < target → left++; sum > target → right--.`,
complexity:`Время: O(n²), Память: O(1)`,
...
steps:`1. Сортировать. Два цикла i, j (с пропуском дубликатов).
2. left=j+1, right=n-1: two pointers для суммы target-nums[i]-nums[j].
3. При нахождении — пропускать дубликаты.`,
complexity:`Время: O(n³), Память: O(1)`,
...
steps:`1. low=0, mid=0, high=n-1.
2. nums[mid]==0: swap(low,mid), low++, mid++.
3. nums[mid]==2: swap(mid,high), high--.
4. nums[mid]==1: mid++.`,
complexity:`Время: O(n), Память: O(1)`,
...
steps:`1. slow и fast стартуют с head.
2. Двигать: slow на 1, fast на 2. При встрече — цикл есть.
3. Сбросить slow в head. Двигать оба по 1 шагу.
4. Новая встреча = начало цикла.`,
complexity:`Время: O(n), Память: O(1)`,
...
steps:`1. slow=nums[0], fast=nums[0].
2. slow=nums[slow], fast=nums[nums[fast]], пока не встретятся.
3. Сбросить slow=nums[0]. Двигать оба по 1 шагу.
4. Встреча = дубликат.`,
complexity:`Время: O(n), Память: O(1)`,
...
steps:`1. Расширять окно right.
2. maxCount = макс. частота в окне.
3. Если (размер окна - maxCount) > k: сужать left.
4. Обновлять result.`,
complexity:`Время: O(n), Память: O(1)`,
...
steps:`1. product=1, left=0.
2. product *= nums[right].
3. Пока product >= k: product /= nums[left], left++.
...
steps:`1. Сортировать. left=0, right=n-1.
2. Если people[l] + people[r] <= limit: l++ (оба в лодку).
3. r-- всегда (самый тяжёлый уплывает).
4. boats++.`,
complexity:`Время: O(n log n), Память: O(1)`,
...
steps:`1. i=0 (first), j=0 (second).
2. lo = max(first[i][0], second[j][0]), hi = min(first[i][1], second[j][1]).
3. Если lo <= hi — добавить пересечение.
4. first[i][1] < second[j][1] → i++, иначе j++.`,
complexity:`Время: O(n+m), Память: O(1)`,
...
steps:`1. zeros=0, left=0.
2. Если nums[right]==0: zeros++.
3. Пока zeros > k: если nums[left]==0 → zeros--, left++.
4. result = max(result, right - left + 1).`,
complexity:`Время: O(n), Память: O(1)`,
complexityExpl:`Один проход — O(n). Только три переменные — O(1).`,
...
steps:`1. i=0 (word1), j=0 (word2).
2. Пока оба не кончились: добавить word1[i], word2[j], i++, j++.
3. Дописать остаток word1 или word2.`,
...
// ===== SLIDING WINDOW =====
{id:"sw14",t:"Maximum Average Subarray I",p:"Sliding Window",d:"легко",
...
steps:`1. Считаем сумму первого окна [0..k-1].
2. Сдвигаем окно: sum += nums[right] − nums[right−k].
3. Обновляем maxSum. Ответ: maxSum / k.`,
complexity:`Время: O(n), Память: O(1)`,
...
steps:`1. target = threshold * k (чтобы не делить).
2. Сумма первого окна, проверяем sum > target.
3. Сдвигаем окно, считаем подходящие.`,
complexity:`Время: O(n), Память: O(1)`,
...
steps:`1. HashSet window, left = 0.
2. Если chars[right] в окне — удаляем chars[left], left++.
3. Добавляем chars[right], обновляем max длину.`,
...
hint:`Два указателя l и r, массив частот на 128 (ASCII). Добавляем s[r], пока частота символа > 1 — сжимаем окно с l. Вклад (r−l+1). Сумму накапливать в long, чтобы не переполнить int.`,
code:`public class Solution {
    public Integer countingPairs(String s) {
        int l = 0;
        long res = 0;
        int[] cnt = new int[128];
...
steps:`1. l = 0, res = 0, cnt[128] = нули.
2. r идёт по строке: cnt[s[r]]++.
3. Пока cnt[s[r]] > 1 — уменьшаем cnt[s[l]], l++.
4. res += (r − l + 1); в конце return (int) res.`,
complexity:`Время: O(n), Память: O(1)`,
complexityExpl:`Каждый символ добавляется и убирается из окна не более одного раза — O(n). Массив фиксированной длины 128 — O(1) доп. памяти.`,
expl:`Окно без повторов: считаем частоты в cnt. При дубликате двигаем l. Число подстрок, заканчивающихся в r с уникальными символами, равно длине окна (r−l+1). Сумма в long защищает от переполнения при длинных строках.`,
