...
code:`/**
 * Run-Length Encoding сжимает строку, заменяя повторяющиеся символы
 * на "символ + количество повторений" (количество только если > 1)
 * 
 * @param s входная строка (только буквы, null безопасен)
 * @return сжатая строка или пустая строка если input null/empty
 * @throws IllegalArgumentException если встречен небуквенный символ
 * 
 * Примеры:
 * rle("aaabbc") → "a3b2c"
 * rle("abc")    → "abc"
...
public String rle(String s) {
    ...
    if (s == null) {
        return "";
        // или throw new IllegalArgumentException("Input cannot be null");
    }
