...
desc:`Дано множество точек на плоскости. Нужно проверить, симметричны ли они относительно вертикальной прямой (оси Y), но с дополнительным строгим условием:
Каждая точка должна иметь ровно одну зеркальную пару
Точка A зеркальна точке B, если они находятся на одинаковом расстоянии от оси, но по разные стороны
Одна точка не может быть парой для нескольких других
Точки, лежащие точно на оси, обрабатываются особо:
Они могут быть парой только сами себе
Но это разрешено только если такая точка всего одна на оси
Если на оси две одинаковые точки — они не считаются парой друг для друга (потому что строгость требует уникальности пары)
...
desc:`==Яндекс.== Даны два ==сжатых вектора== vec1 и vec2 ==одинаковой развёрнутой длины== в виде списков пар [value, count]. Например, [3, 3, 3, 7] — это [[3, 3], [7, 1]]. Посчитать ==скалярное произведение== развёрнутых векторов.
...
desc:`Дан массив парковки: 0 = свободное место, 1 = занятое. Машина занимает ==ровно k мест==. Найти ==начало первого свободного окна== из k подряд идущих нулей, ближайшего к ==центру парковки==. Если таких нет — вернуть -1. При равном расстоянии до центра — вернуть левое.

Пример 1:
Ввод: lot = [1,0,0,0,1,0,0], k = 3
Вывод: 1  (окно [1..3], расстояние до центра 3: |1.5−3|=1.5 vs |5−3|=2)

Пример 2:
Ввод: lot = [1,1,1,1], k = 2
Вывод: -1

Пример 3:
Ввод: lot = [0,0,1,0,0], k = 2
Вывод: 3  (центр=2; окна [0..1] и [3..4] равноудалены → выбираем левое [0..1])
Вывод: 0`,
hint:`Скользящее окно размером k. Считаем нули в окне prefix sum. Окно валидно если нулей == k (т.е. нет единиц). Среди валидных окон выбираем с минимальным расстоянием середины окна до центра парковки.`,
code:`class Solution {
    public int bestParkingSpot(int[] lot, int k) {
        int n = lot.length;
        if (k > n) return -1;

        // prefix[i] = количество нулей в lot[0..i-1]
        int[] prefix = new int[n + 1];
        for (int i = 0; i < n; i++)
            prefix[i + 1] = prefix[i] + (lot[i] == 0 ? 1 : 0);
        ...
        for (int l = 0; l + k <= n; l++) {
            int zeros = prefix[l + k] - prefix[l];
            if (zeros == k) {                           // всё окно свободно
                double mid = l + (k - 1) / 2.0;
                double dist = Math.abs(mid - center);
                if (dist < bestDist) {                  // строго меньше → левое при равенстве
                    bestDist = dist;
                    bestPos = l;
                }
            }
...
complexity:`Время: O(n), Память: O(n)`,
complexityExpl:`Один проход для prefix sum + один проход по окнам — O(n). Массив prefix[n+1] — O(n) памяти.`,
expl:`Prefix sum позволяет за O(1) посчитать нули в любом окне. Окно валидно если zeros==k (нет занятых мест). Среди валидных выбираем с минимальным расстоянием середины до центра. При равном расстоянии побеждает левое — условие строгого < в сравнении.`}
