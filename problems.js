...
code:`class Solution {
    public int[] searchRange(int[] nums, int target) {
        //по сути это два одинаковых метода. но тут ищет первый индекс, что больше или равно таргет
        int left = lowerBound(nums, target);

        //возвращает первый индекс где значение строго больше target, поэтому важно сделать -1
...
desc:`Даны два массива одинаковой длины N (значения — целые числа, в типичной постановке из диапазона 1..N или вообще произвольные). Для каждого индекса i от 0 до N−1 нужно найти размер ==пересечения множеств== значений в префиксах A[0..i] и B[0..i] (каждый префикс рассматриваем как множество **уникальных** чисел, без кратности).
...
complexity:`Время: O(n), Память: O(1)`,
complexityExpl:`Делаем один линейный проход по строке длины n, каждую цифру/оператор обрабатываем константным числом действий, значит время O(n). Используем только переменные num, operator, last и sum, поэтому доп. память O(1).`,
...
complexity:`Время: O(T log K), Память: O(T), где T — суммарное число кодпоинтов, K — число уникальных символов в слове`,
complexityExpl:`Пусть T — суммарное число кодпоинтов во всех строках, а K — число различных символов в одной строке. Для каждого символа обновление в TreeMap стоит O(log K), поэтому суммарно O(T log K). Память O(T): храним группы и построенные ключи для слов.`,
...
complexity:`Время: O(n), Память: O(k), где n — длина строки, k — число различных символов`,
complexityExpl:`Первый проход считает частоты, второй ищет первый символ с частотой 1, суммарно O(n) по времени. В HashMap хранится до k различных символов строки, поэтому память O(k).`,
...
complexity:`Время: O(n * d), Память: O(n)`,
...
complexity:`Время: O(n), Память: O(1)`,
complexityExpl:`Делаем до двух линейных проходов по массиву длины n: сначала переносим ненули, затем дописываем хвост нулями. Суммарно это O(n) по времени. Используются только индексы и временные переменные, поэтому доп. память O(1).`,
...
complexity:`Время: O(n), Память: O(1)`,
complexityExpl:`Указатель right проходит массив один раз, а left тоже движется только вперёд и суммарно делает не более n шагов. Поэтому время O(n). Храним лишь счётчик нулей и границы окна, значит доп. память O(1).`,
...
complexity:`Время: O(n log n + m log m), Память: O(1) без учёта сортировки`,
...
complexity:`Время: O(E log E), Память: O(E)`,
...
complexity:`Время: O(n), Память: O(w), где n — число узлов, w — максимальная ширина дерева`,
complexityExpl:`Каждый из n узлов извлекается и обрабатывается ровно один раз, значит время O(n). В очереди одновременно лежат узлы текущего уровня, максимум w штук (w — ширина дерева), поэтому память O(w).`,
...
complexity:`Время: O(m+n), Память: O(1), где m и n — размеры массивов`,
complexityExpl:`Каждый элемент из nums1 и nums2 рассматривается не более одного раза при движении указателей i, j и k с конца, поэтому время O(m+n). Алгоритм работает in-place, используя только несколько индексов, значит память O(1).`,
...
complexity:`Время: O(n), Память: O(h), где n — число узлов, h — высота дерева`,
complexityExpl:`DFS заходит в каждый из n узлов не более одного раза, поэтому время O(n). Дополнительная память идёт только на стек рекурсии глубиной до h, то есть O(h).`,
...
complexity:`Время: O(n), Память: O(1), где n = |s2|`,
complexityExpl:`Окно длины |s1| сдвигается по s2 один раз, на каждом шаге обновляются константно две частоты, а сравнение массивов длины 26 занимает O(1). Поэтому итоговое время O(n), где n = |s2|. Доп. память O(1), так как массивы частот фиксированного размера.`,
...
complexity:`Время: O(1) на операцию, Память: O(n), где n — число элементов в стеке`,
complexityExpl:`Каждая операция push, pop, top и getMin выполняется за O(1), так как это доступ к вершинам deque без обходов. По памяти в худшем случае стек минимумов может вырасти до n (например, строго убывающая последовательность), значит суммарно O(n).`,
...
complexity:`Время: O(n), Память: O(n), где n — число путей`,
complexityExpl:`Делаем два прохода по списку из n путей: в первом собираем города-источники в HashSet, во втором ищем город назначения. Это O(n) по времени. Множество хранит до n уникальных городов-источников, значит память O(n).`,
...
complexity:`Время: O(log n), Память: O(1), где n — размер массива`,
complexityExpl:`На каждом шаге бинарного поиска отбрасываем примерно половину текущего диапазона, поэтому число итераций O(log n). Используются только индексы left/right/mid, значит доп. память O(1).`,
...
complexity:`Время: O(m+n), Память: O(1), где m и n — длины списков`,
complexityExpl:`Каждый из m+n узлов сравнивается и присоединяется к результату не более одного раза, поэтому время O(m+n). Новые узлы не создаются (кроме dummy), только перекидываются ссылки, значит доп. память O(1).`,
