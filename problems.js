...
expl:`Ключевая идея: если prefix[i] % k == prefix[j] % k, то сумма подмассива (j+1..i) кратна k, потому что (prefix[i] - prefix[j]) % k == 0.
Три важных нюанса:
...
expl:`Для каждого i нужно произведение всех элементов кроме nums[i] = prefix[0..i-1] * suffix[i+1..n-1]. Делить нельзя — может быть ноль. Первый проход слева направо: result[i] = произведение всех элементов левее i. Второй проход справа налево: накапливаем suffix = произведение всех правее i, умножаем result[i] *= suffix. Итог: result[i] = prefix * suffix = всё кроме i. O(n) время, O(1) доп. память.`,
